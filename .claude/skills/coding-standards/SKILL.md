...
description: "所有编码任务的规范。覆盖防御性编程、分层架构(Web)/量化架构(数据科学)、TDD、安全、性能、API设计、数据模型、并发控制、可观测性、代码审查共 10 个模板。生成或修改任何代码时自动激活，逐项校验合规性。模板 2/2a/3 为条件约束，模板 5 限定作用域，其余为硬约束。"
---

## 前置规则：Bug 溯源

代码审查发现 bug 时，必须立即查阅所有关联文档（ADRs、相关脚本 git log、历史讨论），全盘分析变更链后再做出判断。禁止仅凭代码片段下结论。
...
## 前置规则：代码修改清单（每次改代码前逐项确认）

修改任何代码前，按以下顺序逐项确认，缺一不可。完成后在对应项的 ☐ 改为 ☑。

1. ☐ **模板对照**: 确认本次修改涉及模板 1-10 中哪几条，逐条检查合规性
2. ☐ **配置单源** (硬约束): 本次修改是否引入了新的参数数值？如有，必须：
   - 参数定义在 config.yaml 中，含来源注释
   - 代码中通过 cfg("key") 读取，禁止 cfg("key", fallback_default)
   - 禁止任何直接写入的数值字面量（公差常量如 252/500 除外）
3. ☐ **埋点日志** (模板 9 硬约束): 新增/修改的每个函数是否加了 logger？
   - 函数入口: logger.info 含函数名和关键参数 + t0 = time.monotonic()
   - 关键步骤: logger.debug/info 标记主要阶段
   - 函数出口: logger.info 含耗时
   - 异常出口: logger.error/warning 含上下文
   - 并行 worker: 每个 worker 函数内独立 get_logger，不受父进程日志影响
4. ☐ **语法检查**: python -c "import ast; ast.parse(open('file.py').read())"
5. ☐ **小规模快速测试**: n_symbols <= 100 / lookback <= 20，确认无语法/logic 错误再扩量
6. ☐ **输出可诊断**: 如果卡住/静默，能通过日志定位到具体位置（文件:行号:函数）

> 此清单为硬约束。任何未逐项确认的提交不得合并。日志不是写好代码再加，是写代码的一部分。
...
- **线程数上限 (硬约束): `max_workers` 上限 4**
  - 来源: M1 Max 实测 — 4 线程在 SQLite WAL 读并发 + Python GIL 双重约束下吞吐最优
  - 根因: SQLite WAL checkpoint 需等所有读者完成当前事务才推进; 超过 4-6 读者时 WAL 文件膨胀导致读变慢, checkpoint 更慢, 正反馈恶化
  - Python GIL: pandas/numpy 运算释放 GIL, 4 线程可跑满 4 核; 8+ 线程 GIL 争抢显著
  - 禁止: `max_workers = os.cpu_count()` 或在无实测支撑下调高线程数
  - 例外: 网络 IO 密集型 (多 API 数据拉取) 线程数按 API 数设, 不受此限
...
- 进度埋点 (硬约束): 任何持续 >5 秒无输出的操作必须逐步骤打印进度。
  - 循环: 每 N 批打印计数+耗时 | 单步: 开始打 `computing {name}...`，结束打耗时。
  - 应用范围: 因子计算、协方差矩阵、数据拉取循环、任何 >=100 元素的批处理。
...
- 命名：函数动词+名词，布尔 is_/has_/can_，常量全大写，类驼峰
 - 结构：函数 <50 行，Python文件 <800 行，参数 <5 个，嵌套 <4 层，无重复代码
...
**所有参数必须从 config.yaml 读取**（cfg("key")），禁止代码内 cfg("key", fallback_default)。
config.yaml 是唯一数据源。缺失 key → cfg() 返回 None → 运行时 TypeError → fail-fast 立即可定位。
