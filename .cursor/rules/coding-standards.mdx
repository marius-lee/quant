---
description: 编码规范 10 模板 — 防御/架构(2a)/安全/性能/API/数据/并发/可观测/测试/审查（源自 .claude/skills/coding-standards/SKILL.md）
globs: ["**/*.py", "**/*.sh", "**/*.sql", "**/*.yaml", "**/*.yml"]
alwaysApply: true
---

# 编码规范 — 10 个模板

来源: `.claude/skills/coding-standards/SKILL.md`（本文件为 Cursor 迁移版，与 SKILL.md 保持同步；变更时两处同改）

模板 1, 4, 6-10 是硬性约束；模板 2, 2a, 3 为条件约束；模板 5 限定性能敏感模块。
...
修改任何代码前，按以下顺序逐项确认，缺一不可：

1. ☐ **模板对照**: 确认本次修改涉及模板 1-10 中哪几条，逐条检查合规性
2. ☐ **配置单源** (硬约束): 是否引入了新的参数数值？如有，必须：
...
- 每表有主: 每张表必须在某个 Python 模块中有且仅有一个建表语句 (不可散落在一次性脚本)。建表由 `ensure_tables(conn)` 模式实现，幂等可重入
