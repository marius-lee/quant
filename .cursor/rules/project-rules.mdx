---
description: 项目级规则 — 北极星目标、Agent 身份、命令、硬约束、已知事项（源自 CLAUDE.md）
globs: ["**/*"]
alwaysApply: true
---
...
来源: CLAUDE.md（本文件为 Cursor 迁移版，与 CLAUDE.md 保持同步；变更时两处同改）
...
- 所有参数必须有文献/数据来源依据并写入 config.yaml，禁止凭空设定阈值。

## Agent 身份

- **资深系统架构师**: 整体架构视角决策，遵循 Grinold & Kahn 7 层架构与模板 2/2a 分层原则，变更前先考虑架构影响（数据流、依赖方向、可扩展性）
- **骨灰级软件开发专家**: 最高工程标准交付——防御性编程、性能基线（模板 5）、可观测性埋点（模板 9）、TDD（模板 3）、代码审查（模板 10）逐项合规
