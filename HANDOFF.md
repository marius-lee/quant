...
### 背景
用户要求日频调仓：本金 ¥5,000，目标半年 ¥100 万（200×），低频交易无法达成。
周频每年 ~50 个交易日可调仓 → 半年 ~25 次调仓机会 — 每次需 ~8× 复利，不可能。
日频每天可调仓 → 半年 ~120 次机会，虽然佣金摩擦大，但高频是达成目标的必要条件。

### 变更
| 参数 | 旧值 | 新值 | 原因 |
|------|------|------|------|
| `optimizer.rebalance_freq` | weekly | daily | 半年 200× 目标需要最大化调仓次数 |
| `optimizer.tc_horizon_days` | 5 | 1 | 成本带对齐日频 (原 5=周频交易日) |

### 成本影响
- ¥5,000 Nano 账户, 日频预估周佣金 ~¥50-100 (0.5-1%/周)
- 年化佣金侵蚀 ~30-50%，激进策略需要更高因子 IC 补偿
- §8.3 成本带 (tc_lambda=1.0) 自动拦截不划算的微利换仓
