...
def _fetch_value_em(conn: sqlite3.Connection, symbols: list, sleep_ms: int = 500) -> int:
    ...
    symbols: 要查询的股票代码列表
    sleep_ms: 请求间隔 (东方财富频率限制, 默认 500ms)
    ...
    for i, sym in enumerate(symbols):
        ...
        try:
            ...
            if pb is not None and pb >= 0:  # PB 可能为 0 (破净)
                updates.append("pb=?")
    ...
    conn.commit()
    logger.info(f"stock_value_em: {updated} updated, {skipped} skipped (of {len(symbols)})")
...
def sync_all(conn: sqlite3.Connection, max_fetch: int = 100) -> dict:
    ...
    max_fetch: 最多获取股票数, -1 = 全部 (全部 ~5000 只约 40min, 建议分批)。
    默认 100 (约 50 秒)。

    返回: {"count": int}
    """
    # 优先更新成交量活跃的股票 (按 amount 降序取 top N)
    rows = conn.execute("""
    ...
    logger.info(f"fundamental sync: {len(symbols)} stocks via stock_value_em (~{est_sec:.0f}s estimated)")
