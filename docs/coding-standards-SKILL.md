...
description: >
  所有编码任务的强制规范。覆盖防御性编程、分层架构(Web)/量化架构(数据科学)、
  TDD、安全、性能、API 设计、数据模型、并发控制、可观测性、代码审查共 10 个模板。
  生成或修改任何代码时自动激活，根据项目类型选择合适的架构模板。
---

# 强制编码规范 — 10 个模板

以下模板是硬性约束。**生成或修改任何代码前，逐项验证合规性。不合规的代码不得输出。**
...
**适用范围**: Web 服务、API、多用户系统。量化/数据科学项目使用模板 2a（见下文）。
...
**生成或修改任何代码前：**
1. 对照 10 个模板逐项校验（模板 2/2a 根据项目类型二选一）
2. 发现不合规 → 修复后再输出
3. 无法确定是否合规 → 在代码注释中标注待确认项

**架构模板选择规则：**
- Web 服务/多用户/API → 模板 2（DDD 四层）
- 量化研究/数据分析/单用户科学计算 → 模板 2a（IO-计算分离）
- 不确定时默认 2a，仅在明确需要多用户/多服务时启用 2
