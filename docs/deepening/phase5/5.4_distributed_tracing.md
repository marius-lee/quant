# Phase 5.4 完成归档 — 分布式追踪

## 变更摘要
集成 OpenTelemetry SDK，实现端到端分布式追踪，支持 Jaeger/Tempo 后端，W3C TraceContext 传播。

## 核心组件

### 1. TracingManager — 追踪管理器
```python
# quant/monitoring/tracing.py
...
def traced(name: str = None, kind: SpanKind = None, attributes: Dict[str, Any] = None):
    """函数追踪装饰器."""
...
## 代码变更
- 新增 `quant/monitoring/tracing.py` - TracingManager
- 新增 `quant/monitoring/tracing_config.py` - 配置类
- 依赖: `opentelemetry-api`, `opentelemetry-sdk`, `opentelemetry-exporter-otlp`, `opentelemetry-instrumentation-requests`, `opentelemetry-instrumentation-sqlite3`
