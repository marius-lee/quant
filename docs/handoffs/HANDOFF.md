...
### 背景
本次会话修了三个问题交织在一起，需要合并记录:

1. **benchmark.py API 切换**: Sina 数据源被 DNS 屏蔽 → 切到 eastmoney (`stock_zh_index_daily_em`)
2. **CSI 300 基准数据**: 同步 `benchmark_daily` 表 (9 行新数据), 使 `residual_momentum_126d` / `idio_vol_126d` 可以物化
3. **文档审计**: 澄清回测各步骤的因子范围, 消除混乱

### factor_cache 状态 (2026-07-19)
| 指标 | 值 |
|------|-----|
| 因子数 | 55 (全部 backtesting) |
| 行数 | 46,402,664 |
| 大小 | ~7.0 GB |
...
| 状态 | 数量 | 参与范围 |
|------|------|----------|
| active | 8 | smoke_test + 实盘信号 |
| monitoring | 29 | 实盘信号 (衰减权重) |
| retired | 26 | backtesting 池 |
| rejected | 7 | 永久排除 (5 DATA_SPARSE + 2) |
| **backtesting** | **55** | run_diagnostics + eval_standard + 增量物化 |
| **using** | **37** | 实盘 (active + monitoring) |

### 回测三部曲的因子范围对照表
| 步骤 | 脚本 | 因子范围 | 因子数 | 用途 |
|------|------|----------|--------|------|
| 1. 诊断 | `run_diagnostics.py` | backtesting | 55 | IC快照 → Phase 2预筛 |
| 2. 冒烟 | `smoke_test.py` | active | 8 | 验证管线不崩 |
| 3. 正式 | `eval_standard.sh` | backtesting | 55 | Phase 1→5 完整因子认证 |

**关键区分**: 冒烟测试用 active (8), 诊断和正式评估用 backtesting (55)。
两者职责不同, 不可混淆。冒烟验证链路通畅, 正式评估验证因子质量。

### 修改清单
| 文件 | 改动 |
|------|------|
| `quant/data/benchmark.py` | `stock_zh_index_daily` → `stock_zh_index_daily_em` (Sina→eastmoney) |
| `scripts/smoke_test.py` | 头注释: "2个"→"8个", 补充职责说明 |
| `web/app.py` | VERSION → test-v158 |

### 因子物化 cron (2026-07-19 更新)
```
0 6 * * 6  # 每周六 06:00 增量物化因子
```
增量逻辑: `materialize(dates, factors, symbols, force=False)` → 每日期只算缺失因子, 已有因子跳过.

### 前置条件 (新机器重建)
1. 拉取 CSI 300 基准: `PYTHONPATH=. .venv/bin/python3 scripts/sync_benchmark.py`
2. 物化因子: `PYTHONPATH=. .venv/bin/python3 scripts/materialize_factors.py`
---
# HANDOFF — 2026-07-17 (test-v141)

## test-v141: CPCV 数据不足优雅降级 — 对齐业界标准

### 问题
test-v140 的日历修复后，diagnostics passed 从 29 降到 2。eval_standard.sh 跑 Phase 2 仅 1 个因子（volatility_126d），其 IC 序列只有 2 个日期。
Phase 3 的 `cpcv.py:47` 触发 `ValueError: Not enough dates (2) for 5-fold CPCV`，Phase 4→Phase 5 也随之中断。

### 业界标准
- RiceQuant/米筐: IC 历史 < 1 年 → 标记 `insufficient_data`，因子不被拒绝
- QuantConnect/LEAN: 样本不足 → 降级验证或标记 "inconclusive"
- De Prado (2018): 日期 < K×2 → 减少折数或跳过，但不拒绝因子

共同原则: **永不崩溃。** 数据不足是暂时状态，不是因子失效。

### 修改 (4 文件)

#### 1. cpcv.py — ValueError → WARNING + fallback
- `n < n_groups*2`: `raise ValueError` → `logger.warning` + 继续走到 fallback split
- 60/40 fallback: 加强 `n < 4` 防御（连简单 split 都不够时返回空 splits 而非崩溃）

#### 2. phase3_oos.py — 日期数不足时优雅降级
- `unique_dates` < 4 → 跳过 OOS 验证
- 所有 candidates 通过（kept=全量），OOS_ICIR 写 0.0 占位
- 写入 `note: "insufficient_data: {N} factor(s) passed through Phase 3 due to insufficient IC history ({M} dates < 4 min for CPCV fold). OOS validation deferred to next evaluation cycle."`
- evaluation_runs 保留完整记录

#### 3. phase4_costs.py — 识别 OOS_ICIR=0.0 占位值
- 读取 `p3.note` 中的 `insufficient_data` 标记
- OOS_ICIR=0.0 时:
  - 跳过净 Sharpe 计算
  - 因子同时加入 `final_factors`（标记已处理）和 `dropped`（标记 OOS 数据不足）
  - `sharpe_estimates[name]["note"]` 写明 "awaiting more data"
- insufficient_data 标记沿传到 evaluation_runs

#### 4. phase5_monitor.py — insufficient_data 因子处理
- 读取 `p3.note` 和 `p4.insufficient_data`
- 识别 insufficient_data 因子: {p3_kept ∩ p4_final} 中标记 insufficient_data 的
- 这些因子 → `status='monitoring'` (非 rejected)
- `status_reason`: "Phase 3: OOS validation skipped — IC history too short for CPCV. IC={val}. Awaiting more data for re-evaluation."
- 下次评估周期数据攒够后, monitoring 因子通过 backtesting 集合重新进入完整流程

### 数据流
```
Phase 2 (1 factor) → Phase 3 (date check: 2<4 → insufficient_data, keep=1)
    → Phase 4 (OOS_ICIR=0.0 placeholder → skip net SR, pass through)
    → Phase 5 (insufficient_data → monitoring, not rejected)
    → Next cycle: monitoring→backtesting→re-evaluate with more data
```

### 版本: test-v141
\n\n---
# HANDOFF — 2026-07-17 (test-v140)

## test-v140: 评估管线 5 项修复 — 状态同步 + 相关性矩阵 + 交易日历

### 改动 1: Phase 5 sync — monitoring 因子独立状态
**文件**: `quant/evaluation/phase5_monitor.py`

- monitoring 因子（ICIR 0.15-0.25）→ `status='monitoring'`，带 IC/ICIR 数值的 reason
- 彻底淘汰因子（ICIR<0.15 且/或 |IC|<0.01）→ `status='rejected'`
- 诊断预筛排除因子 → 保留原状态不变（对齐业界标准：预筛选不是最终判决）
- 新增 `diagnostics_excluded` 日志输出

### 改动 2: 相关性矩阵修复
**文件**: `quant/factor/stats_cache.py`

- 从 `factor_cache.db` 读取因子值填充 `factor_values_by_date`（删除"不再需要"的空赋）
- 修复 test-v139 引入的全零相关性矩阵 bug
- 使用 `FactorStore.load()` 逐日加载因子值，仅保留 ≥30 只股票覆盖的截面

### 改动 3: IC 计算日期对齐
**文件**: `quant/factor/ic.py`

- `pd.bdate_range()`（美国日历）→ `pd.date_range(freq="B")` + `is_trading_day()`（中国A股日历）
- data 加载后追加 `trading_days ∩ data.index.unique()` 安全过滤
- 消除春节期间 `_compute_one_day` KeyError（2026-02-16~2026-02-23）

### 改动 4: OOS 验证日期对齐
**文件**: `quant/scheduler/oos_verify.py`

- 两处 `pd.bdate_range()` → `pd.date_range(freq="B")` + `is_trading_day()` 过滤

### 改动 5: Pipeline ZTD 预热日期对齐
**文件**: `quant/pipeline.py`

- `pd.date_range(freq="B")` → + `is_trading_day()` 过滤
- 低风险：仅缓存预热，日期缺失不导致崩溃

### 日期对齐问题扩散范围
| 文件 | 修复方式 |
|------|---------|
| `ic.py:96` | ✅ `is_trading_day()` + data 交集 |
| `oos_verify.py:46,66` | ✅ `is_trading_day()` |
| `pipeline.py:177` | ✅ `is_trading_day()` |
| `loop.py:200` | 无需修复（已有 `is_trading_day()` 过滤） |
| `stats_cache.py:105` | 无需修复（SQL 查 `daily` 表） |
| `factor_cache.py:26` | 无需修复（SQL 查 `daily` 表） |

### 设计原则
- monitoring 因子不在 Phase 5 标为 rejected，等待实盘归因模块（attribution）根据 IC 衰减降级
- 降级到 retired → 自动回归 `backtesting` 集合 → 下次评估周期重新考核
- 诊断预筛排除的因子保留原状态，不永久标记为 rejected（业界标准：QuantConnect/RiceQuant/AQR）
