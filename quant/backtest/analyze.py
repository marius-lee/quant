...
从 diagnostics.py 拆出, 专注策略回测后的分析:
  - FactorTracker: 逐日跟踪因子贡献
  - diagnose(): 回测后诊断, 输出因子权重调整建议
  - apply_diagnosis(): 根据诊断调整 IC 权重

与 diagnostics.py 的分工:
  - diagnostics.py: 因子层 — compute_pre_backtest_ic() (回测前 IC 评估)
  - analyze.py:     策略层 — FactorTracker / diagnose / apply_diagnosis (回测后分析)

对标: Quantopian Pyfolio (独立于 Zipline 的 post-backtest 分析)
"""
...
_log = get_logger("backtest.analyze")
