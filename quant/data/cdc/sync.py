...
设计:
  - 订阅 ChangeCapture 变更事件
  - 批量 UPSERT 到 DuckDB (列存, 并行查询优势)
  - 因子缓存增量失效/重算
  - 审计表实时更新
  - 幂等保证: 基于 LSN 去重 + 主键冲突处理
"""
...
logger = get_logger("data.cdc.sync")
...
class IncrementalSyncer:
    ...
    def _handle_changes(self, events: List[Any]):
        ...
        for event in events:
            ...
            try:
                if event.op.value == "INSERT" or event.op.value == "UPDATE":
                    self._upsert_row(table, mapping, event.new_row)
                    result.rows_upserted += 1
                elif event.op.value == "DELETE":
                ...
                logger.error(f"Sync failed for {table} event {event.lsn}: {e}")
                result.success = False
                result.failed_events.append({"lsn": event.lsn, "error": str(e)})

        # 更新位点
        if events:
            max_lsn = max(e.lsn for e in events)
            self._position_manager.set_position(f"cdc_sync:{events[0].table}", max_lsn)
    ...
    def _run_loop(self):
        ...
        while self._running:
            try:
                if self.config.enable_duckdb and self._duckdb_conn:
                    self._duckdb_conn.commit()  # 显式提交
            except Exception as e:
