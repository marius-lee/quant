...
数据源:
  baostock 行业分类 (证监会行业分类)。单次无 code 查询仅返回前 500 只
  (BAOSTOCK_PER_PAGE_COUNT), 故必须逐股查询.
...
try:
    from quant.utils.baostock_gate import (
        bs_query as _bs_query, BaostockBlacklisted, BaostockQuotaExceeded,
    )
...
def _symbol_intervals(code: str, start_date: str, today: str,
                      probe, global_days: set = None) -> list:
    ...
    if ind0 == ind1 and ind0 is not None:
        mid = (date.fromisoformat(start_date)
               + (date.fromisoformat(today) - date.fromisoformat(start_date)) // 2).isoformat()
        qm = probe(code, mid)
        if qm is None or qm[1] == ind0:
            return [(start_date, ind0)]
        # 中点回归 (罕见) → 落入变更路径
    ...
    # ── 变更路径: 锚点扫描 + 全局变更日剪枝二分 ──
    anchors = _anchors(start_date, today)
    ...
    fr = snap[first_idx][0]
    if first_idx > 0:
        _fr = _first_record(start_date, fr)
        if _fr is not None:
            fr = _fr
