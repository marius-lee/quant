...
def fetch_quotes(symbols: list[str]) -> dict[str, dict]:
    ...
    if len(batches) > 1:
        ...
        try:
            futures = {ex.submit(_fetch_tencent_batch, b): i for i, b in enumerate(batches)}
            for f in as_completed(futures):
                result.update(f.result())
        finally:
            ex.shutdown(wait=False)
    else:
        result = _fetch_tencent_batch(batches[0])
...
def _fetch_tencent_batch(batch: list[str]) -> dict[str, dict]:
    """腾讯实时行情单批拉取."""
