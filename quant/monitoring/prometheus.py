...
class QuantMetrics:
    ...
    BACKTEST_SHARPE = Gauge(
        "quant_backtest_sharpe", "Backtest Sharpe ratio",
        ["strategy", "run_id"],
        registry=_metrics_registry,
    )
    BACKTEST_CAGR = Gauge(
        "quant_backtest_cagr_pct", "Backtest CAGR percentage",
        ["strategy", "run_id"],
        registry=_metrics_registry,
    )
    BACKTEST_MAX_DD = Gauge(
        "quant_backtest_max_drawdown_pct", "Backtest max drawdown",
        ["strategy", "run_id"],
        registry=_metrics_registry,
    )
    BACKTEST_DSR = Gauge(
        "quant_backtest_dsr", "Backtest Deflated Sharpe Ratio",
        ["strategy", "run_id"],
        registry=_metrics_registry,
    )
...
class MetricsCollector:
    ...
    def _collect_business_metrics(self):
        ...
        try:
            ...
            # Data rows
            for table in ["daily", "stocks", "daily_valuation", "financial_income", "financial_balance", "financial_cashflow"]:
            ...
            _log.debug(f"Business metrics collection error: {e}")
...
class GrafanaDashboardBuilder:
    """生成 Grafana Dashboard JSON (可直接导入 Grafana)."""
                ...
                "panels": [
                    {
                        ...
                        "targets": [{"expr": "quant_total_equity", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Daily PnL",
                        "type": "graph",
                        "gridPos": {"x": 12, "y": 0, "w": 12, "h": 8},
                        "targets": [{"expr": "rate(quant_trade_pnl_total[1h])", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Drawdown %",
                        "type": "graph",
                        "gridPos": {"x": 0, "y": 8, "w": 12, "h": 8},
                        "targets": [{"expr": "quant_drawdown_pct", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Sharpe Ratio",
                        "type": "stat",
                        "gridPos": {"x": 12, "y": 8, "w": 6, "h": 4},
                        "targets": [{"expr": "quant_backtest_sharpe", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Max Drawdown %",
                        "type": "stat",
                        "gridPos": {"x": 18, "y": 8, "w": 6, "h": 4},
                        "targets": [{"expr": "quant_max_drawdown_pct", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Active Positions",
                        "type": "table",
                        "gridPos": {"x": 0, "y": 16, "w": 24, "h": 8},
                        "targets": [{"expr": "quant_position_value > 0", "format": "table"}],
                    },
                ],
                ...
                "panels": [
                    {
                        ...
                        "type": "graph",
                        "gridPos": {"x": 0, "y": 0, "w": 12, "h": 8},
                        "targets": [{"expr": "quant_factor_ic", "legendFormat": "{{factor_name}}"}],
                    },
                    {
                        "title": "Factor ICIR",
                        "type": "graph",
                        "gridPos": {"x": 12, "y": 0, "w": 12, "h": 8},
                        "targets": [{"expr": "quant_factor_icir", "legendFormat": "{{factor_name}}"}],
                    },
                    {
                        "title": "Factor Rank",
                        "type": "heatmap",
                        "gridPos": {"x": 0, "y": 8, "w": 24, "h": 8},
                        "targets": [{"expr": "quant_factor_rank", "format": "heatmap"}],
                    },
                ],
                ...
                "panels": [
                    {
                        ...
                        "targets": [{"expr": "quant_var_95", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Max Drawdown",
                        "type": "graph",
                    ...
                    },
                    {
                        ...
                        "targets": [{"expr": "quant_leverage", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Turnover Ratio",
                        "type": "graph",
                        "gridPos": {"x": 0, "y": 8, "w": 8, "h": 8},
                        "targets": [{"expr": "quant_turnover_ratio", "legendFormat": "{{strategy}}"}],
                    },
                    {
                        "title": "Concentration",
                        "type": "graph",
                        "gridPos": {"x": 8, "y": 8, "w": 8, "h": 8},
                        "targets": [{"expr": "quant_concentration", "legendFormat": "{{strategy}}/{{symbol}}"}],
                    },
                    {
                        "title": "VaR 99%",
                        "type": "graph",
                        "gridPos": {"x": 16, "y": 8, "w": 8, "h": 8},
                        "targets": [{"expr": "quant_var_99", "legendFormat": "{{strategy}}"}],
                    },
                ],
                "time": {"from": "now-1d", "to": "now"},
                "refresh": "30s",
    ...
    def export_all(cls, output_dir: str = "grafana_dashboards"):
        ...
        dashboards = {
            "overview.json": cls.build_overview_dashboard(),
            "factors.json": cls.build_factor_dashboard(),
            "risk.json": cls.build_risk_dashboard(),
        }
        for name, content in dashboards.items():
            path = os.path.join(output_dir, name)
            with open(path, "w") as f:
                json.dump(content, f, indent=2, ensure_ascii=False)
        _log.info(f"Grafana dashboards exported to {output_dir}")
...
class MonitoringPlatform:
    ...
    def start(self):
        ...
        _log.info("MonitoringPlatform started")
