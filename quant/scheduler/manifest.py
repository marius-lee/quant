...
_DAYLINE: list[TaskSpec] = [
    ...
    TaskSpec(
        ...
        # (尾盘5分钟增量量能 = 14:55 + 15:00 两次快照取差 — v429 判定: 无因子消费
        #  此量能, 按零冗余原则不实现; 若未来注册尾盘增量类因子再启)
    ...
    TaskSpec(
        name="daily_repair", label="早间补拉", schedule="05:00",
        # v562c: 窗口 08:00-08:30 → 06:00-08:30 — 实测 fund_flow 单表 ~50min
        # (500 只 × 5s 东财限流), 6 表串行 1.5-2h > 30min 窗口. 历史连续 4 天
        # (08-15~08-19) 全在 08:30 被 grace_s=1800 标 aborted, 从未成功.
        # v562d: 05:00 起跑 — 周六 orchestrator 阻塞等 daily_repair 完成
        # (runners.py run_daily_repair) 才 spawn weekly_eval (06:00), 06:00 起跑
        # 会压缩周六评估窗口; 05:00 起跑 07:00 前完成, weekly_eval 正常启动.
        window=(time(5, 0), time(8, 30)), grace_s=10800, timeout_s=10800,
