...
from factor.synth import sleeve_compose, ic_weighted, equal_weight
...
class TestSleeveCompose:
    ...
    def test_basic_sleeve(self):
        ...
        result = sleeve_compose(fv, positions_per_factor=2)
    ...
    def test_empty_factor_values(self):
        result = sleeve_compose({}, min_factors=0)
        assert len(result) == 0

    def test_too_few_factors(self):
        fv = {'f1': pd.Series([0.9], index=['A'])}
        result = sleeve_compose(fv, min_factors=2)
        assert len(result) == 0
    ...
    def test_nan_handling(self):
        ...
        result = sleeve_compose(fv, positions_per_factor=1)
